...
def getStats():
    ...
    logbookWList = cursor.fetchone()
    
    cursor.execute("SELECT count(added) FROM timestamps, logbook WHERE endTime = 'N/A' AND timestamps.uniqueID = logbook.uniqueID AND timestamps.dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    logbookNew = cursor.fetchone()
    ...
    cursor.execute("SELECT added, arrived, endTime, noShow, cancelled, flushedFromWaitlist FROM timestamps WHERE dateAdded BETWEEN ? AND ?", (dateFrom, dateTo,))
    sumTime = 0
    totalPeople = 0
    ...
    data["logBook"] = logbookWList[0] + logbookNew[0]
